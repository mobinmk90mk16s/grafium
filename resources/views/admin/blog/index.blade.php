...
<head>
    ...
    <title>مدیریت بلاگ | GRAFIUM</title>
...
<body>
        ...
        <nav class="space-y-5">
            <div>
                ...
                <ul class="space-y-0.5">
                    <li>
                        <a href="{{ route('admin.blog.dashboard') }}" class="menu-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94a3b8] text-sm font-medium transition">
                            <i data-lucide="layout-dashboard" class="w-5 h-5 text-[#60a5fa]"></i> داشبورد بلاگ
                            <span class="menu-indicator"></span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.blog.posts') }}" class="menu-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94a3b8] text-sm font-medium transition">
    ...
    <main class="mr-[270px] p-6 min-h-screen main-content">
        ...
        <div class="flex flex-wrap justify-between items-center pb-4 border-b border-[#1a2f4a] mb-6">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-2xl font-extrabold text-white">مدیریت بلاگ</h1>
                </div>
                <p class="text-sm text-[#475569] mt-0.5 mr-9">مدیریت پست‌ها، دسته‌بندی‌ها و نظرات</p>
            </div>
        ...
        <div class="stats-banner">
            ...
            <div class="stat-item">
                <span class="num">{{ $stats['draft'] ?? 0 }}</span>
            ...
            <div class="stat-item">
                <span class="num">{{ $stats['total_comments'] ?? 0 }}</span>
        ...
        <div class="filter-bar">
            <input type="text" id="searchInput" placeholder="جستجو در پست‌ها..." onkeyup="filterTable()" class="w-48" />
            ...
            <a href="{{ route('admin.blog.create') }}" class="btn-emerald mr-auto">
                <i data-lucide="plus" class="w-4 h-4"></i> پست جدید
            </a>
        </div>
                ...
                <table id="blogTable">
                    <thead>
                        <tr>
                            ...
                            <th style="min-width:60px;">تصویر</th>
                            <th style="min-width:150px;">عنوان</th>
                            <th style="min-width:100px;">دسته‌بندی</th>
                            <th style="min-width:80px;">نویسنده</th>
                            <th style="min-width:100px;">تاریخ</th>
                            <th style="min-width:80px;">وضعیت</th>
                            <th style="min-width:80px;">بازدید</th>
                            <th style="min-width:120px;">عملیات</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($posts as $post)
                        <tr data-status="{{ $post->status }}" data-search="{{ $post->title }} {{ $post->summary }}">
                            <td>{{ $loop->iteration }}</td>
                            <td>
                                @if($post->media)
                                    <img src="{{ asset('storage/' . $post->media) }}" alt="{{ $post->title }}" class="w-10 h-10 rounded-lg object-cover" />
                                @else
                            ...
                            <td class="font-medium text-right">{{ $post->title }}</td>
                            <td>
                                @if($post->category)
                                    <span class="text-xs px-2 py-1 rounded-full" style="background: {{ $post->category->color }}20; color: {{ $post->category->color }}; border: 1px solid {{ $post->category->color }}30;">
                                        {{ $post->category->name }}
                                    </span>
                                @else
                                    <span class="text-xs text-[#475569]">دسته‌بندی نشده</span>
                                @endif
                            </td>
                            <td>{{ $post->author->name ?? 'نامشخص' }}</td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <button onclick="toggleFeatured({{ $post->id }})" class="{{ $post->is_featured ? 'text-amber-400 hover:text-amber-300' : 'text-[#475569] hover:text-amber-400' }} transition">
                        ...
                        <tr>
                            <td colspan="9" class="text-center py-12 text-[#475569]">
                                <i data-lucide="file-text" class="w-12 h-12 mx-auto text-[#475569] mb-3"></i>
                                <p>هیچ پستی یافت نشد</p>
                                <p class="text-xs mt-1">برای ایجاد پست جدید، روی دکمه "پست جدید" کلیک کنید</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="pagination-bar">
            <span class="page-info">نمایش {{ $posts->firstItem() ?? 0 }} - {{ $posts->lastItem() ?? 0 }} از {{ $posts->total() }} پست</span>
            <div class="page-btns">
                {{ $posts->links() }}
            </div>
        </div>
    ...
    <script>
        ...
        function toggleFeatured(id) {
            fetch(`/admin/blog/${id}/toggle-featured`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    showToast('وضعیت ویژه با موفقیت تغییر کرد', 'success');
