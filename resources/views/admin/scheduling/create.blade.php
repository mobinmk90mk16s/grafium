...
@section('title', 'افزودن زمان‌بندی جدید | GRAFIUM')
...
<div class="flex flex-wrap justify-between items-center pb-4 border-b border-[#1a2f4a] mb-6">
    <div>
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-2xl font-extrabold text-white">افزودن زمان‌بندی جدید</h1>
        </div>
        <p class="text-sm text-[#475569] mt-0.5 mr-9">ایجاد یک زمان‌بندی جدید برای رزروها</p>
    </div>
...
<!-- ===== FORM ===== -->
<div class="form-card">
    <form action="{{ route('admin.scheduling.store') }}" method="POST" id="schedulingForm">
        @csrf
        <input type="hidden" name="service_item_id" id="serviceItemInput" value="">
        <input type="hidden" name="date_time" id="dateTimeInput" value="">
        <input type="hidden" name="end_time" id="endTimeInput" value="">
        ...
        <div class="form-group">
            ...
            <div class="select-box" id="datePickerTrigger" style="opacity:0.5;pointer-events:none;">
                <span class="placeholder" id="datePickerText">ابتدا آیتم خدمت را انتخاب کنید</span>
        ...
        <div class="form-group" id="statusSection" style="display:none;">
            ...
            <div class="status-grid">
                <div class="status-card status-available selected" data-status="available">
                    <i data-lucide="check-circle"></i>
                    <span class="label">قابل رزرو</span>
                </div>
                <div class="status-card status-reserved" data-status="reserved">
                    <i data-lucide="clock"></i>
                    <span class="label">رزرو شده</span>
                </div>
                <div class="status-card status-maintenance" data-status="maintenance">
                    <i data-lucide="wrench"></i>
                    <span class="label">تعمیرات</span>
                </div>
                <div class="status-card status-blocked" data-status="blocked">
                    <i data-lucide="ban"></i>
        ...
        <!-- ===== Step 5: توضیحات ===== -->
        <div class="form-group" id="noteSection" style="display:none;">
            <label><i data-lucide="file-text"></i> توضیحات (اختیاری)</label>
            <textarea name="note" class="input-dark" rows="3" placeholder="توضیحات اضافی..."></textarea>
        </div>
...
<script>
    ...
    // ============================================================
    // تبدیل دقیق میلادی به شمسی (الگوریتم استاندارد)
    // ============================================================
    ...
    function getCurrentPersianDate() {
        ...
        return { year: j.jy, month: j.jm - 1, day: j.jd }; // month صفر-مبنا
    }
    ...
    const state = {
        ...
        currentMonth: todayJalali.month, // صفر-مبنا
    ...
    serviceItemSelect.addEventListener('change', function () {
        ...
        datePickerText.textContent = 'برای انتخاب کلیک کنید';
    ...
    function renderCalendar() {
        ...
        // شنبه = 0 ... جمعه = 6
        // در JS: یکشنبه=0, دوشنبه=1, ..., شنبه=6
        // تبدیل: شنبه→0, یکشنبه→1, ..., جمعه→6
        let firstWeekday = gDate.getDay(); // 0=یکشنبه ... 6=شنبه
        ...
        for (let d = 1; d <= month.days; d++) {
            ...
            // گذشته؟ (تاریخ قبل از امروز)
            const todayStart = new Date();
            todayStart.setHours(0, 0, 0, 0);
            if (jsDate < todayStart) {
